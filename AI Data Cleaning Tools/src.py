...
def dataset_collation_prompt(dataset_list, research_question): 
    ...
    * Out of all the datasets provided, selects the relevant datasets based on the list of csv files. Specify which ones are using by the clean_csv_path.
    * if the question requires time series, identify the datasets that have data for successive time periods and present them together.
    * Suggest data cleaning operations to ensure the datasets are in a usable format.
    * If multiple tables in CSV, suggest to split tables and process separately.
    * Suggest standartizing column and row naming conventions of the datasets with short, distinct names.
    * Suggest data join order if multiple datasets need to be united into one (e.g. to show the full timeseries for multiple years)

    It should be in plain text with some pandas functions suggested (no code examples needed).

    In addition, provide a short description of each column in the resulting dataframe / dataframes using the revised column names.
